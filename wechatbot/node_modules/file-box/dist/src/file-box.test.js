...
tstest_1.test('File smoke testing', async (t) => {
...
class TestFileBox {
    static testFileCreateLocal(localFileFixture) {
        ...
        tstest_1.test('File.createLocal()', async (t) => {
            t.ok(file, 'ok');
        });
        tstest_1.test('File.fromRemote()', async (t) => {
...
tstest_1.test('toBase64()', async (t) => {
...
tstest_1.test('fromBuffer() & toBase64()', async (t) => {
    const BASE64_ENCODED = 'RmlsZUJveEJhc2U2NAo=';
...
tstest_1.test('syncRemoteName()', async (t) => {
...
tstest_1.test('fromURL() deal with url with querystring', async (t) => {
...
tstest_1.test('toDataURL()', async (t) => {
...
tstest_1.test('metadata', async (t) => {
    const FILE_PATH = 'tests/fixtures/data.bin';
...
tstest_1.test('fromQRCode()', async (t) => {
...
tstest_1.test('toQRCode()', async (t) => {
...
tstest_1.test('toJSON() for not supported type', async (t) => {
    const BASE64_ENCODED = 'RmlsZUJveEJhc2U2NAo=';
    const buffer = Buffer.from(BASE64_ENCODED, 'base64');
    const fileBox = file_box_1.FileBox.fromBuffer(buffer, 'test.txt');
 ...
 */
tstest_1.test('toStream() twice for a stream', async (t) => {
    const stream = fs_1.default.createReadStream(__filename);
    const box = file_box_1.FileBox.fromStream(stream, __filename);
    // consume it
    await box.toBase64();
    try {
        await box.toBuffer();
