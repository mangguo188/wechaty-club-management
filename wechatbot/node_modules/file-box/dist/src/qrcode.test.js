...
tstest_1.test('imageBase64ToQrCode()', async (t) => {
    ...
    const text = await qrcode_1.bufferToQrValue(buf);
    t.equal(text, EXPECTED_QRCODE_TEXT, 'should decode qrcode image base64');
});
tstest_1.test('qrValueToStream()', async (t) => {
    ...
    const stream = await qrcode_1.qrValueToStream(QRCODE_VALUE);
    const chunks = [];
