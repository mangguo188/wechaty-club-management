...
tstest_1.default('unpackFileBoxFromChunk()', async (t) => {
    const FILE_BOX_DATA = 'test';
    const FILE_BOX_NAME = 'test.dat';
    const stream = await getFileBoxStreamStub(FILE_BOX_DATA, FILE_BOX_NAME);
    const decodedFileBox = await file_box_chunk_1.unpackFileBoxFromChunk(stream);
...
tstest_1.default('packFileBoxToChunk()', async (t) => {
    const FILE_BOX_DATA = 'test';
    const FILE_BOX_NAME = 'test.dat';
    const fileBox = wechaty_puppet_1.FileBox.fromBuffer(Buffer.from(FILE_BOX_DATA), FILE_BOX_NAME);
    const stream = await file_box_chunk_1.packFileBoxToChunk(fileBox);
    const fileBoxChunk = await next_data_1.nextData(stream);
...
tstest_1.default('packFileBoxToChunk() <-> unpackFileBoxFromChunk()', async (t) => {
    const FILE_BOX_DATA = 'test';
    const FILE_BOX_NAME = 'test.dat';
    const fileBox = wechaty_puppet_1.FileBox.fromBuffer(Buffer.from(FILE_BOX_DATA), FILE_BOX_NAME);
    const stream = await file_box_chunk_1.packFileBoxToChunk(fileBox);
    const restoredBox = await file_box_chunk_1.unpackFileBoxFromChunk(stream);
...
tstest_1.default('should handle no name error in catch', async (t) => {
    t.plan(1);
    const FILE_BOX_DATA = 'test';
    const FILE_BOX_NAME = 'test.dat';
    const stream = await getFileBoxStreamStub(FILE_BOX_DATA, FILE_BOX_NAME, true);
    try {
        await file_box_chunk_1.unpackFileBoxFromChunk(stream);
    }
    catch (e) {
        t.equal(e.message, 'no name');
    }
});
tstest_1.default('should handle first error catch', async (t) => {
    t.plan(1);
    const FILE_BOX_DATA = 'test';
    const FILE_BOX_NAME = 'test.dat';
...
tstest_1.default('should handle middle error in further ops', async (t) => {
    t.plan(1);
    const FILE_BOX_DATA = 'test';
    const FILE_BOX_NAME = 'test.dat';
    const stream = await getFileBoxStreamStub(FILE_BOX_DATA, FILE_BOX_NAME, false, false, true);
    const fileBox = await file_box_chunk_1.unpackFileBoxFromChunk(stream);
    try {
...
const getFileBoxStreamStub = async (data, name, noname = false, firstException = false, middleException = false) => {
