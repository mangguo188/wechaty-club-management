...
tstest_1.test('integration testing', async (t) => {
    ...
    server.addService(mod_1.PuppetService, testServer);
    await util_1.default.promisify(server.bindAsync.bind(server))(ENDPOINT, mod_1.grpc.ServerCredentials.createInsecure());
    server.start();
    /**
     * Create Client
     */
    const client = new mod_1.PuppetClient(ENDPOINT, mod_1.grpc.credentials.createInsecure());
    /**
     * gRPC: Stream
     */
    ...
    for (const data of DING_DATA_LIST) {
        const request = new mod_1.DingRequest();
